...
public class ProvideStudentFeedbackActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        feedbackRef = FirebaseDatabase.getInstance().getReference("studentFeedbacks");
    ...
    private void submitFeedback() {
        ...
        String authorName = currentUser.getDisplayName() != null ? currentUser.getDisplayName() : "Anonymous Student";
        long timestamp = System.currentTimeMillis();

        Feedback feedback = new Feedback(feedbackId, authorId, authorName, "student", feedbackText, timestamp);

        if (feedbackId != null) {
            feedbackRef.child(feedbackId).setValue(feedback)
                .addOnSuccessListener(aVoid -> {
                    Toast.makeText(ProvideStudentFeedbackActivity.this, "Feedback submitted successfully", Toast.LENGTH_SHORT).show();
                    finish();
                })
                .addOnFailureListener(e -> Toast.makeText(ProvideStudentFeedbackActivity.this, "Failed to submit feedback", Toast.LENGTH_SHORT).show());
        }
    }
